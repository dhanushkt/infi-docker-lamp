...
echo "========================================================================"
echo "Starting custom .SQL import script"

if [ "$IMPORT_CUSTOM_DB" = "true" ]; then
    echo "IMPORT_CUSTOM_DB is set to true, proceeding with .sql import"
    if [ -f "/docker-entrypoint-initdb.d/database.sql" ]; then
        mysql -uadmin -p"$PASS" < /docker-entrypoint-initdb.d/database.sql
    else
        echo "database.sql File does not exist in directory /docker-entrypoint-initdb.d"
    fi
elif [ "$IMPORT_CUSTOM_DB" = "false" ]; then
    echo "IMPORT_CUSTOM_DB is set to false, skipping .sql import"
else
    echo "IMPORT_CUSTOM_DB is not set to true or false"
fi

echo ".SQB import script completed"
echo "========================================================================"
